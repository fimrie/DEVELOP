...
class ChemModel(object):
    ...
    def __init__(self, args):
        ...
        # Structural data:
        self.train_struct_data = self.load_structural_data(params['train_struct_file'])
    ...
    def load_data(self, file_name, is_training_data: bool):
        ...
        restrict = self.args.get("--restrict_data") # Added int() as docopt was seeing as string
    ...
    def load_structural_data(self, file_name):
        full_path = os.path.join(self.data_dir, file_name)

        print("Loading types file from %s" % full_path)
        structural_data = []
        with open(full_path, 'r') as f:
            for line in f:
                structural_data.append(line.strip())

        restrict = self.args.get("--restrict_data") # Added int() as docopt was seeing as string
        if restrict is not None and int(restrict) > 0:
            structural_data = structural_data[:int(restrict)]
    ...
    def make_model(self):
        ...
        # Calculate CNN featurisation
        self.ops['cnn_features'] = self.setup_cnn(self.weights, self.placeholders['cnn_input'], "test")
    ...
    def run_epoch(self, epoch_name: str, epoch_num, data, struct_data, is_training: bool):
        loss = 0
        ...
        with open('epoch_types_file_%d' % epoch_num, 'w') as f:
            for d in structural_data:
                f.write(d+'\n')

        # Make minbatch iterator
        batch_iterator = ThreadedIterator(self.make_minibatch_iterator(bucketed, bucket_sizes, bucket_at_step, is_training), max_queue_size=5)
        ...
        e.populate('epoch_types_file_%d' % epoch_num)
        ...
        for step, batch_data in enumerate(batch_iterator):
            ...
            batch_data[self.placeholders['z_prior_in']] = utils.generate_std_normal(\
                self.params['batch_size'], batch_data[self.placeholders['num_vertices']],self.params['hidden_size']) # Two noise vectors

            # Load CNN data
            batch = e.next_batch(self.params['batch_size'])
            gmaker.forward(batch, input_tensor, random_translation=1.0, random_rotation=True)
            batch.extract_label(0, labels_grid)
            if self.params['cnn_ligand_only']:
                cnn_input = input_tensor.tonumpy()[:, 14:, :, :, :]
            else:
                cnn_input = input_tensor.tonumpy()
            batch_data[self.placeholders['cnn_input']] = cnn_input
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                if not self.params['generation']:
                    ...
                    train_losses, train_speed = self.run_epoch("epoch %i (training)" % epoch, epoch,
                                                                                     self.train_data, self.train_struct_data, True)
                    ...
                    valid_losses, valid_speed = self.run_epoch("epoch %i (validation)" % epoch, epoch,
                                                                                     self.valid_data, self.valid_struct_data, False)
                ...
                if epoch >= self.params['epoch_to_generate']:
                    self.generate_new_graphs(self.valid_data, self.valid_struct_data)
                    break
