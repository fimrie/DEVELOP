...
def align_frag_to_full(mol, smi_linker, smi_frags, dataset, prot, save_idx, sdwriter):
    ...
    Arguments
      ...
      - smi_linker: the linker with exit vectors
      - smi_frags: the fragments with exit vectors
    """
    try:
        ...
        # Detect number of dummy atoms in linker
        q = rdqueries.AtomNumEqualsQueryAtom(0)
        num_dummy_atoms = len(linker.GetAtomsMatchingQuery(q))
        ...
        for i, amol in enumerate(aligned_mols):
            sub_idx = list(range(aligned_mols[1].GetNumHeavyAtoms()+num_dummy_atoms))
            ...
            if i == 0:
                ...
                mol_range = list(range(amol.GetNumHeavyAtoms()+num_dummy_atoms))
        ...
        # If want to remove exit vectors, uncomment
        #du = Chem.MolFromSmiles('*')
        #aligned_mols[1] = Chem.RemoveHs(AllChem.ReplaceSubstructs(aligned_mols[1],du,Chem.MolFromSmiles('[H]'),True)[0])
        #aligned_mols[1].SetProp("_Name", Chem.MolToSmiles(aligned_mols[0]))
        
        # Save conformer of fragment
        if prot == '':
            # Name format: [DATASET]_SET_[IDX]
            #aligned_mols[1].SetProp("_Name", dataset+"_SET_" + str(save_idx))
            # Name format: [SMI_FULL].[SMI_FRAGS] - for constrained conf
            #aligned_mols[1].SetProp("_Name", Chem.MolToSmiles(mol)+'.'+smi_frags)
            # Name
            aligned_mols[1].SetProp("_Name", smi_linker+'.'+Chem.MolToSmiles(mol))
        else:
            aligned_mols[1].SetProp("_Name", dataset+"_SET_" + str(save_idx))
        sdwriter.write(aligned_mols[1])
...
def save_linker_pharacophore(mol, smi_frags, smi_linker, dataset, prot, save_idx, sdwriter):
    """ Saves the pharmacophore of the linker (and exit vectors).
    Arguments
      - mol: the reference molecule
      - smi_linker: the linker with exit vectors
      - smi_frags: the fragments with exit vectors
    ...
    try:
        ...
        # Loop over matches
        for linker_match, frag_match in product(linker_matches, frags_matches):
            # Check if match
            l_match = [idx for num, idx in enumerate(linker_match) if linker.GetAtomWithIdx(num).GetAtomicNum() != 0]
            f_match = [idx for num, idx in enumerate(frag_match) if frags.GetAtomWithIdx(num).GetAtomicNum() != 0 and idx not in l_match]
            if len(set(list(f_match)+list(l_match))) == mol.GetNumHeavyAtoms():
                break
        # Add frag indices
        sub_idx += frag_match
        ...
        # Get linker pharmacophores
        coords = []

        ref_conf = aligned_mols[1].GetConformer()
        for i, atom in enumerate(aligned_mols[1].GetAtoms()):
            if atom.GetAtomicNum() == 0:
                coords.append(list(ref_conf.GetAtomPosition(i)))


        pharms = ['Donor', 'Acceptor', 'Aromatic']
        # Get linker pharmacophores
        feats = factory.GetFeaturesForMol(aligned_mols[0])
        num_atoms = aligned_mols[1].GetNumHeavyAtoms()
        proc_feats = []
        for feat in feats:
            if feat.GetFamily() in pharms:
                proc_feats.append([feat.GetFamily(),feat.GetAtomIds()])

        symbol = ['O', 'N', 'C']
        # Create map - aromatic is average position
        fake_lig = '*.*'
        for ent in proc_feats:
            tmp_coords = []
            tmp_idxs = []
            for idx in ent[1]:
                if idx < num_atoms:
                    tmp_coords.append(list(ref_conf.GetAtomPosition(idx)))
                    tmp_idxs.append(idx)
            if len(tmp_idxs) > 0 :
                fake_lig += '.'+symbol[pharms.index(ent[0])]
                coords.append(list(np.average(tmp_coords, axis=0)))

        # Create object
        mol_pharm = Chem.MolFromSmiles(fake_lig)
        conf = Chem.Conformer(mol_pharm.GetNumAtoms())
        for i in range(len(coords)):
            conf.SetAtomPosition(i, coords[i])
        conf.SetId(0)
        cid = mol_pharm.AddConformer(conf)
        # Save conformer of fragment
        if prot == '':
            mol_pharm.SetProp("_Name", smi_linker+'.'+Chem.MolToSmiles(mol))
        else:
            mol_pharm.SetProp("_Name", dataset+"_SET_" + str(save_idx))
        sdwriter.write(mol_pharm)
...
def save_linker_pharmacophore_elaboration(mol, smi_frag, smi_linker, dataset, prot, save_idx, sdwriter):
    """ Saves the pharmacophore of the elaboration (and exit vectors).
    Arguments
      - mol: the reference molecule
      - smi_linker: the elaboration with exit vector
      - smi_frags: the scaffold with exit vector
    """
	# Add second exit vector to fragments
    ...
    try:
        ...
        for atom in aligned_mols[1].GetAtoms():
            if atom.GetAtomicNum() == 0:
                dummy_idx.append(atom.GetIdx())
        for i, amol in enumerate(aligned_mols):
            sub_idx = list(range(aligned_mols[1].GetNumHeavyAtoms()+1))
            for idx in dummy_idx:
                sub_idx.remove(idx)
                sub_idx.append(idx)
            if i == 0:
                ...
                mol_range = list(range(amol.GetNumHeavyAtoms()+1))
            idx_to_add = list(set(mol_range).difference(set(sub_idx)))
            sub_idx.extend(idx_to_add)
            aligned_mols[i] = Chem.rdmolops.RenumberAtoms(amol, sub_idx)
        ...
        #Instead of taking the positions of the dummy atoms, we take the position of one dummy atom
        #and the atom in the molecule which is furthest away from it.
        ref_conf = aligned_mols[1].GetConformer()
        for i, atom in enumerate(aligned_mols[1].GetAtoms()):
            if atom.GetAtomicNum() == 0 and atom.GetDegree() > 0: #i.e. this is the true exit vector because it's connected to other atoms
                coords.append(list(ref_conf.GetAtomPosition(i))) #We want the coords of this dummy atom
        ...
        # Pharmacophores need an atom type for libmolgrid (arbitrary)
        symbol = ['O', 'N', 'C'] # {O: Donor, N: Acceptor, C: Aromatic}
        # Create map - aromatic feature is average position of ring
        #fake_lig = '*.*'
        fake_lig = '*'
...
def create_frags_pharma_sdf_dataset(fragmentations, path_to_conformers, dataset, sdffile, sdffile_pharm, pdbbind_ids=[], prot='', verbose=False):
    if dataset not in ["ZINC", "CASF", "PDBbind"]:
        print("Dataset must be either ZINC or CASF or PDBbind")
    ...
    for toks in fragmentations:
        if toks[0] in dataset_dict:
            dataset_dict[toks[0]].append([toks[1], toks[2], toks[3], toks[4]])#+'.'+toks[3]])
        else:
            dataset_dict[toks[0]] = [[toks[1], toks[2], toks[3], toks[4]]]#+'.'+toks[3]]]
    ...
    if sdwriter is None:
        ...
        sys.exit(-1)
    # Setup SD writer pharm
    outf_pharm = open(sdffile_pharm,'w+')
    sdwriter_pharm = Chem.SDWriter(outf_pharm)
    if sdwriter_pharm is None:
        print("Could not open ".sdffile_pharm)
        sys.exit(-1)
        
    # Initialise placeholder for results
    fragmentations_new = []
    ...
    for count, mol in enumerate(conformers):
        ...
        if mol is not None:
            ...
            if mol_name in dataset_dict and (mol.GetProp("_Name") in pdbbind_ids or dataset is not "PDBbind"):
                # Loop over all fragmentations of this mol
                for fragments in dataset_dict[mol_name]:
                    success = align_frag_to_full(mol, fragments[0], fragments[1], dataset, prot, save_idx, sdwriter)
                    success_pharm = save_linker_pharacophore(mol, fragments[0], fragments[1], dataset, prot, save_idx, sdwriter_pharm)
                    if success:
        ...
        if verbose:
            # Progress
            if count % 1000 == 0:
                print("\rProt: %s, Mol: %d, Successes: %d" % (prot, count, len(fragmentations_new)), end = '')

    if verbose:
        print("\rDone")
        if fail_count_conf>0:
            print("Fail count conf %d" % fail_count_conf)
    
    return fragmentations_new, (fail_count, fail_count_conf, fails)
