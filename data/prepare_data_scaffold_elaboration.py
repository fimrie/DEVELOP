...
"""
Usage:
    prepare_data_scaffold_elaboration.py [options]
...
from align_utils import align_mol_to_frags_elaboration
...
def read_file(file_path, add_idx=False, calc_pharm_counts=False):
    ...
    for i, line in enumerate(lines):
        ...
        if len(toks) == 1:
            ...
            smi_mol, smi_elab, smi_frags, idx = toks
        else:
...
def preprocess(raw_data, dataset, name, output_dir='', test=False):
    ...
    processed_data =[]
    total = len(raw_data)
    for i, (smi_mol, smi_frags, smi_elab, abs_dist, idx) in enumerate([(mol['smi_mol'], mol['smi_frags'], 
                                                                        mol['smi_elab'], mol['abs_dist'], mol['idx']) for mol in raw_data]):
        if test:
            smi_mol = smi_frags
            smi_elab = ''
...
if __name__ == "__main__":
    ...
    if args.get('--data_path') and args.get('--dataset_name'):
        ...
        data_paths = ['scaffold_elaboration/data_zinc_train.txt', 'scaffold_elaboration/data_zinc_valid.txt', 'scaffold_elaboration/data_casf.txt', 'scaffold_elaboration/data_pdbbind_2019_refined.txt']
        names = ['zinc_train', 'zinc_valid', 'casf_test', 'pdbbind_test']
